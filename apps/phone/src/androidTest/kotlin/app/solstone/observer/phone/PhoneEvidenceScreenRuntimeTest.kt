...
class PhoneEvidenceScreenRuntimeTest {
    @After
    fun resetHooks() {
        PhoneHarnessRuntime.hooks = null
    }
    ...
    fun roomSeededEvidenceAndStatusRenderOnUiThread() {
        ...
        PhoneHarnessRuntime.hooks = PhoneRuntimeHooks().also { hooks ->
    ...
    fun failedEvidenceReadRendersDistinctErrorState() {
        ...
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            ...
            assertTrue(recoveryLatch.await(10, TimeUnit.SECONDS))
            scenario.onActivity {
                PhoneHarnessRuntime.container!!.close()
            }
            evidenceLatch = CountDownLatch(1)
            PhoneHarnessRuntime.hooks?.onEvidenceLoadComplete = { evidenceLatch.countDown() }
            scenario.onActivity { activity ->
                clickButton(activity.findViewById(android.R.id.content), "Evidence + export")
            }
            assertTrue(evidenceLatch.await(10, TimeUnit.SECONDS))
